	...
	for (let element of data) {
		if (element.type === 'way' && element.geometry && element.geometry.length > 1) {
			...
			if (['motorway', 'trunk', 'primary'].includes(element.tags?.highway)) {
				console.log(element.tags.name || 'Unnamed road', 'was skipped because no parking is allowed there!');
				continue;
			}
			...
			if (mult <= 0) {
				console.log(element.tags.name || 'Unnamed road', 'was skipped because no parking is allowed on either side!');
				continue;
			}
			
			if (element.tags.access) {
				console.log('Access', element.tags.name || 'unnamed', element.tags.access);
			}
			if (element.tags.vehicle) {
				console.log('Vehicle', element.tags.name || 'unnamed', element.tags.vehicle);
			}
			if (element.tags.motor_vehicle) {
				console.log('Motor', element.tags.name || 'unnamed', element.tags.motor_vehicle);
			}
			if (element.tags.car) {
				console.log('Motor', element.tags.name || 'unnamed', element.tags.car);
			}
			
			console.log(element.tags.name || 'Unnamed road', `parking allowed on ${mult} sides of the road!`);
